...
GetKey(){                                                               # type dd, wait for input and...
   ...
   second=`dd bs=1 count=1 2>/dev/null`; case "$second" in '['|'0'|'O') # intercept second one (different for some consoles)
    third=`dd bs=1 count=1 2>/dev/null`; case "$third" in               # intercept the third character in a string
                        ...
                        C|OC) first=RI;;                                # decision for up
                        D|OD) first=LE;;                                # decision for down arrow-keys
...
if [[ "$cursor" = "UP" ]]; then echo UP; fi
if [[ "$cursor" = "DN" ]]; then echo DOWN; fi
if [[ "$cursor" = "LE" ]]; then echo LEFT; fi                        # some confused behavior after script ends.
if [[ "$cursor" = "RI" ]]; then echo RIGHT; fi